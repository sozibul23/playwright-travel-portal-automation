...
(async () => {
  ...
  await page.locator('div, li, button, span, [role="option"]').filter({ hasText: /\bDAC\b/i }).filter({ visible: true }).first().click({ force: true });
  await page.waitForTimeout(500);
  ...
  await page.waitForTimeout(800);
  await page.locator('div, li, button, span, [role="option"]').filter({ hasText: /\bCXB\b/i }).filter({ visible: true }).first().click({ force: true });
  await page.waitForTimeout(500);
  ...
  console.log('5. Results page URL:', page.url());
